...
"""
struct LegacyXfoil{Ta, Tre, Tma, Txt, Tri, Tit, Tnp, Tpm, Tpd, Tzi, Tcmx, Tcmn} <: Method
    alpha::Ta             
    re::Tre           
    mach::Tma       
    xtrip::Txt
...
# Default Arguments
- `alpha::AbstractArray{Float}` : angle of attack (in degrees)
...
function LegacyXfoil(alpha, re, mach; xtrip=(1.0, 1.0), reinit=false, iter=50, npan=140, percussive_maintenance=!reinit, printdata=false, zeroinit=true, clmaxstop=false, clminstop=false)
    return LegacyXfoil(alpha, re, mach, xtrip, reinit, iter, npan, percussive_maintenance, printdata, zeroinit, clmaxstop, clminstop)
end
