...
# Returns
- `method::NeuralFoil` : NeuralFoil method object with fields:
## NeuralFoil Object Fields:
...
- `Re::Vector{Float}` : Reynolds number(s) at which to analyze
- `Ma::Float` : Mach number for which to apply corrections
- `n_crit::Float` : value for e^n model
- `xtr_upper::Float` : upper side forced transition location
- `xtr_lower::Float` : lower side forced transition location
"""
function NeuralFoil(
    reynolds=1e6, mach=0.0; model_size="xlarge", n_crit=9.0, xtr_upper=1.0, xtr_lower=1.0
)
