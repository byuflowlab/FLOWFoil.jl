...
"""
function naca65(
    cli, sub_series="65-x10"; N=161, slope_tolerance=1e-6, a=1.0, smoothing=false
)
    ...
    for i in 1:N
        if x[i] < 0.005
            if leading_edge_radius^2 -
               ((leading_edge_circle_center_x - x[i]) / cos_theta[2])^2 < 0
                d = 0.0
            else
                d = sqrt(
            ...
            x_upper[i] = x[i] - d * sin_theta[i]
            x_lower[i] = x[i] + d * sin_theta[i]

            y_upper_1 = x[i] * tan_theta[i] + d * cos_theta[i]
            y_upper_2 = y_c[i] + y_t[i] * cos_theta[i]

            y_lower_1 = x[i] * tan_theta[i] - d * cos_theta[i]
